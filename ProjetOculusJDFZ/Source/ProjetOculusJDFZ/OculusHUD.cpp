	...
	: Super(PCIP)
{
	...
	static ConstructorHelpers::FObjectFinder<UFont>HUDFontOb(TEXT(
...
void AOculusHUD::DrawHUD()
{
	...
	AProjetOculusJDFZGameMode* MyGameMode = Cast<AProjetOculusJDFZGameMode>(
		UGameplayStatics::GetGameMode(this));
	if (MyGameMode->GetCurrentState() == 
		EOculusProjectPlayStats::EInitialization)
	{
		FString InstructionString = TEXT(
			"Hello, press X button/front left button/Enter to start the simulation");
		//create a variable for storing the size of printing our instruction
		FVector2D instructionSize;
		GetTextSize(InstructionString, instructionSize.X,
			instructionSize.Y, HUDFont);
		DrawText(InstructionString, FColor::Yellow,
			(ScreenDimension.X - instructionSize.X) / 2.0f,
			(ScreenDimension.Y - instructionSize.Y) / 2.0f, HUDFont);
	}
}
